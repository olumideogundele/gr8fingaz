...
$information =  mysqli_real_escape_string($conn,$_POST['information']);
  ...
  $extract_user = mysqli_query($conn, "SELECT * FROM `users_investment_type` WHERE  (`name` = '$name')") or die(mysqli_error($conn));
...
$sql = 	 "INSERT INTO `users_investment_type`(`name`, `maturity`,   `investment`, `information`, `created_date`, `registeredby`, `status` ) VALUES
...
$information =  mysqli_real_escape_string($conn,$_POST['information']);
  ...
  $extract_user = mysqli_query($conn, "SELECT * FROM `users_investment_type` WHERE  (`id` = '$e_id')") or die(mysqli_error($conn));
		$count = mysqli_num_rows($extract_user);
		 if ($count > 0) {
	 	 
		 
	 
$sql = 	 "UPDATE `users_investment_type` SET `name` = '$name', `maturity` = ' $maturity', `investment` = '$investment', `information` = '$information', `registeredby` = '$emailing', `status` = '$status'  WHERE `id` = '$e_id'";
